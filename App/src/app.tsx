...
const App = (): JSX.Element => {
  ...
  const [buttonLayout, setButtonLayout] = useState([
    { i: 'exit', x: 9, y: 0, w: 1, h: 0.305, static: true }
  ]);
  const [rawData, setRawData] = useState(null);
  const layout = [
    { i: 'menu_buttons', x: 1, y: 0, w: 4, h: 0.3, static: true },
  ...
  useEffect(() => {
    ...
    ipcRenderer.invoke('get-api-data').then(async (menu_data) => {
      ...
      for (const [key, menu_object] of Object.entries(menu_data)) {
        let x = 0;
        let y = 0;
        setButtonLayout((oldArray) => [
          ...oldArray,
          { i: key, x: layoutX, y: 0, w: 1, h: 0.305, static: true }
        ]);
        layoutX = layoutX + 1;
        const new_menu_object = menu_object as menu; // Cast type to object

        // Map objects to JSX Nodes
        const complete_menu: Array<JSX.Element> = new_menu_object.map(
          (data: menu_item) => {
            // Vars
            data.menu = key;
            ...
            return (
              <div
                className={'menu-item'}
                style={{}}
                onClick={() => setOrder((oldArray) => [...oldArray, data])}
                key={id}
              >
                <img
                  alt={'menu item image'}
        ...
        <GridLayout measureBeforeMount={true} layout={layout} cols={5}>
          <div key={'menu_buttons'} className={'button-layer'}>
            <GridLayout layout={buttonLayout} cols={9}>
              {data != null ? (
                data.map(
                  (item: { menu_name: string; nodes: Array<JSX.Element> }) => {
                    return (
                      <NavLink
                ...
                (
                  ...
                  if (index === 0) {
                    return (
                      <>
                        ...
                        <Route
                          key={item.menu_name}
                          path={`/menus/${item.menu_name}/items`}
                          component={() => (
                            <Menu objects={rawData} items={item} />
                          )}
                        />
                      </>
                    );
                  ...
                  return (
                    <Route
                      key={item.menu_name}
                      path={`/menus/${item.menu_name}/items`}
                      component={() => <Menu objects={rawData} items={item} />}
                    />
                  );
                }
              )
